...
scaling_factor = 2
playerImg = pygame.transform.scale(playerImg, (playerImg.get_width() * scaling_factor, playerImg.get_height() * scaling_factor))
playerX = 1
playerY = 37
playerY_change = 0
playerX_change = 0
...
def get_cord():
    x = random.randint(0, (screen.get_width() - 130))
    y = random.randint(53, (screen.get_height() - 300))
    return x, y
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_ESCAPE:
                ...
                timer_thread.join()
                # show_game_over()
                # # tmp_timer_thread = threading.Thread(target=timer_count())
                ...
                # replay_rect = replay_text.get_rect(center=(a_x // 3, a_y - 100))
                # exit_rect = exit_text.get_rect(center=(2 * a_x // 3, a_y - 100))
